...
namespace Xtrim_ERP.gui
{
    public partial class FrmDebtorDeduct : Form
    {
        ...
        private void initGrfBill()
        {
            ...
            panel3.Controls.Add(grfBill);

            theme1.SetTheme(grfBill, "VS2013Green");
            grfBill.Cols[colID].Visible = false;
            grfBill.Cols[colItemId].Visible = false;
        }
        private void initGrfDeduct()
        {
            grfDeduct = new C1FlexGrid();
            grfDeduct.Font = fEdit;
            grfDeduct.Dock = System.Windows.Forms.DockStyle.Fill;
            grfDeduct.Location = new System.Drawing.Point(0, 0);
            grfDeduct.Rows.Count = 1;
            grfDeduct.Cols.Count = 6;
            grfDeduct.Cols[colItemNameT].Width = 220;
            grfDeduct.Cols[colExpn].Width = 100;
            grfDeduct.Cols[colimcome].Width = 100;
            //grfBill.Cols[colCDrawDate].Width = 100;
            //grfBill.Cols[colCAmt].Width = 100;
            grfDeduct.Cols[colItemNameT].Caption = "รายการ";
            grfDeduct.Cols[colExpn].Caption = "ค่าใช้จ่าย";
            grfDeduct.Cols[colimcome].Caption = "รายได้";
            //grfBill.Cols[colCDrawDate].Caption = "วันที่";
            //grfBill.Cols[colCAmt].Caption = "รวมเงิน";

            panel4.Controls.Add(grfDeduct);

            theme1.SetTheme(grfDeduct, "VS2013Red");
            grfDeduct.Cols[colID].Visible = false;
            grfDeduct.Cols[colItemId].Visible = false;
        }
        ...
        private void setRowGrfDeduct(Items itm)
        {
            if (itm.item_name_t == null) return;
            Row rowB = grfDeduct.Rows.Add();
            rowB[colID] = "";
