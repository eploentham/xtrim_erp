...
{
    public partial class FrmItemsSubType : Form
    {
        XtrimControl xC;
        ItemsType expnT;
        Font fEdit, fEditB;
        ...
        public FrmItemsSubType(XtrimControl x)
        {
            InitializeComponent();
            xC = x;
            initConfig();
        ...
        {
            grfExpnT = new C1FlexGrid();
            grfExpnT.Font = fEdit;
            grfExpnT.Dock = System.Windows.Forms.DockStyle.Fill;
            grfExpnT.Location = new System.Drawing.Point(0, 0);

            FilterRow fr = new FilterRow(grfExpnT);

            grfExpnT.AfterRowColChange += new C1.Win.C1FlexGrid.RangeEventHandler(this.grfDept_AfterRowColChange);
            //grfExpnC.CellButtonClick += new C1.Win.C1FlexGrid.RowColEventHandler(this.grfDept_CellButtonClick);
            //grfExpnC.CellChanged += new C1.Win.C1FlexGrid.RowColEventHandler(this.grfDept_CellChanged);

            panel2.Controls.Add(grfExpnT);

            theme1.SetTheme(grfExpnT, "Office2013Red");
        }
        private void setGrfDeptH()
        {
            //grfDept.Rows.Count = 7;
            grfExpnT.DataSource = xC.xtDB.expntDB.selectAll();
            grfExpnT.Cols.Count = 5;
            TextBox txt = new TextBox();

            grfExpnT.Cols[colCode].Editor = txt;
            grfExpnT.Cols[colName].Editor = txt;
            grfExpnT.Cols[colRemark].Editor = txt;

            grfExpnT.Cols[colCode].Width = 60;
            grfExpnT.Cols[colName].Width = 200;
            grfExpnT.Cols[colRemark].Width = 100;

            grfExpnT.ShowCursor = true;
            //grdFlex.Cols[colID].Caption = "no";
            //grfDept.Cols[colCode].Caption = "รหัส";

            grfExpnT.Cols[colCode].Caption = "รหัส";
            grfExpnT.Cols[colName].Caption = "ชื่อ ประเภท ค่าใช้จ่าย";
            grfExpnT.Cols[colRemark].Caption = "หมายเหตุ";
            Color color = ColorTranslator.FromHtml(xC.iniC.grfRowColor);
            for (int i = 1; i < grfExpnT.Rows.Count; i++)
            {
                grfExpnT[i, 0] = i;
                if (i % 2 == 0)
                    grfExpnT.Rows[i].StyleNew.BackColor = color;
            }
            //CellRange rg1 = grfBank.GetCellRange(1, colE, grfBank.Rows.Count, colE);
            //rg1.Style = grfBank.Styles["date"];
            grfExpnT.Cols[colID].Visible = false;
        }
        ...
        private void grfDept_AfterRowColChange(object sender, C1.Win.C1FlexGrid.RangeEventArgs e)
        {
            ...
            String deptId = "";
            deptId = grfExpnT[e.NewRange.r1, colID] != null ? grfExpnT[e.NewRange.r1, colID].ToString() : "";
            setControl(deptId);
            setControlEnable(false);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                setDeptment();
                String re = xC.xtDB.expntDB.insertExpenseType(expnT, xC.user.staff_id);
                int chk = 0;
